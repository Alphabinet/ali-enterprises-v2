...
const STATIC_SLIDE: BaseSlide = {
    id: "static-hero-1",
    type: "image",
    src: STATIC_SLIDE_SRC,
    alt: "Ali Enterprises - Industrial Solutions"
};
    ...
    render: () => (
      <div className="flex flex-col items-center justify-center h-full w-full relative z-10 animate-in fade-in zoom-in duration-700">
        ...
        <div className="w-full max-w-[90%] lg:max-w-[80%] bg-white/10 backdrop-blur-md border border-white/10 rounded-2xl p-4 lg:p-6 text-center shadow-xl">
           <h2 className="text-xl lg:text-4xl font-extrabold text-white tracking-tight">Hasan Ali</h2>
           <p className="text-amber-400 font-bold uppercase text-[10px] lg:text-xs tracking-[0.2em] mb-1">Managing Director</p>
           <div className="flex flex-col items-center gap-1 text-teal-100 text-xs lg:text-sm font-medium border-t border-white/10 pt-2">
             <span className="flex items-center gap-1.5"><span className="w-1 h-1 rounded-full bg-teal-400"></span> Ali Enterprises</span>
             <span className="flex items-center gap-1.5 opacity-80"><MapPin size={12} /> Khatauli, U.P.</span>
           </div>
        </div>
        <div className="mt-2">
           <a href="/contactus" className="inline-flex items-center gap-2 text-white font-bold text-xs lg:text-sm border-b border-amber-400/50 pb-0.5 hover:text-amber-400 hover:border-amber-400 transition-all">Get in Touch <ArrowRight size={14} /></a>
        </div>
      </div>
    )
  },
    ...
    render: () => (
      <div className="flex flex-col items-center justify-center h-full w-full px-6 relative z-10 animate-in zoom-in duration-500 text-center">
        ...
        <div className="relative">
           ...
           <p className="text-teal-50 text-sm lg:text-lg leading-relaxed font-medium max-w-xs mx-auto italic">"To revolutionize the manufacturing sector with affordable, <span className="text-amber-400 not-italic font-bold bg-amber-400/10 px-1 rounded">AI-driven</span> automated solutions."</p>
           <Quote className="absolute -bottom-4 -right-4 text-teal-500/30 w-8 h-8" />
        </div>
        <div className="mt-8">
          <a href="/products" className="bg-white text-teal-900 px-6 py-2.5 rounded-full font-bold text-sm shadow-lg hover:bg-teal-50 transition-colors flex items-center gap-2">View Technology <ArrowRight size={16} /></a>
        </div>
      </div>
    )
  }
...
// --- Main Hero Component ---
const HeroSection = () => {
  const [slides, setSlides] = useState<BaseSlide[]>([STATIC_SLIDE]);
                ...
                <div className="relative w-full h-full bg-slate-900">
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-90">
                      <div className="absolute bottom-6 left-5 lg:bottom-12 lg:left-10 max-w-[90%] lg:max-w-[80%]">
                          ...
                          <div 
                            className={`mt-3 h-1 w-12 bg-teal-500 rounded-full transition-all duration-700 delay-200 
                            ${idx === 0 ? 'w-20 opacity-100' : (currentIndex === idx ? 'w-20 opacity-100' : 'w-0 opacity-0')}`}
                          ></div>
                      </div>
                  </div>
                </div>
